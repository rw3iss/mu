...
Can we try to add a third 'split mode', for the global movie player (is something like a different 'mini' mode).
Show a new icon in the full global player overlay, in the top bar, to the right of the existing 'minimize' icon button in the upper left. Make an identical button that looks like a 'split view' icon. and when clicked, enable this split mode.
The split mode should operate as follows:
- The movie player will become an overlay on the right side of the screen, taking up half the screen (but let this be configurable, we will customize it later). The 'top control' bar, that previously had the minimize, split  mode, and close icon, should still show here, as the top row on the split view, with the minimize button, 'full mode' button, and then the close icon on the right, still. Clicking either button should enable that state (ie. mini view, full mode, or close the player entirely).
- It would be convenient to allow the user to 'drag' the split center bar, and resize the split mode. That would be nice. By default, when opening, it should be 50% page width (or 50vw). The width should be persisted to the local storage ui settings, and reloaded on refresh, if set and still in 'split mode'. When the split mode width is set, also calculate the size and position of where the video overlay should belong relative to it (ie. set it's width to the split view right column width, and it's height to the 16:9 ratio accordingly), and persist those in local storage as well, or come up with an easy way to calculate them from the split view defined width, upon restoration. This value should define an offset to place the video, relatively, and also for the title and other content to start below it, in the split view right column. Update the movies size and aspect ratio, and this offset, if the user drags or changes the split view size. The global player will need some kind of flag to wrap itself in 'split view', or signal to the main app also to show in 'split view', so that the main app can 'crop' its views to the width of where the split view starts, so it emulates a "split view".
- The movie will play in the right column, with the movie at the top (it will be fixed or absolute, like it is now in the other views), taking up the full width of the column, and it's height should be set to what the 16:9 ratio is for the split view on the current users screen (ie. half the "9 side", so 4.5x the unit). Below the movie, show the seek bar, with the the title and time below it. The title can go on the left, and the time on the right side, in a single row, both static in position as well below the movie, and then below that, show a scrollable container with all of the basic details about the movie (ie. what shows in the 'info' overlay, etc, that the user can read about the movie while watching it in this view).
- In the split view, the player controls will not take up the full width at the bottom. Instead, they should be constrained to the split view's right side's width, and still show at the bottom, in a fixed location, to control the movie. It should hide the title and time here, in the player controls, in this mode, as they will be shove above in the top below the movie in it's info. Instead, in this view, the player control should move the 'centered' back/play/pause/forward buttons to be left-aligned, on the left side of the player controls, when in split view. Override the central control positioning when in this mode to have them set to the left, beginning of the split view.

When in split mode, the right side will be the open player, and the user will be free to navigate the site on the left side. Ensure the split mode signals to the main app, and sets a flag for initialization, that the app should crop or constrict its width to the defined split view width. Then the user can continue browsing with all of the content showing on the left.

Ensure these values are updated live, and the site functions smoothly, whenever the user resizes the split view, or otherwise enables or disables it to another mode, or closes the player entirely.

Implement the new split mode into the global player as defined, improving and ensuring the player usability as you see fit, but keeping all functionality and styistic approaches intact.