...
| Module | Purpose |
|--------|---------|
| `auth` | JWT authentication, user sessions. localBypass works only during initial setup; once a user exists, every request needs a JWT cookie. **Self-registration** (`registration.service.ts`): three admin switches stored as app settings — `registrationAllow` (default **off**), `registrationRequireApproval` (default on), `registrationRequireEmailVerification` (default off) — edited in Settings → Users. Public `GET /auth/registration-config` (drives the login page's Register button), admin `PUT /auth/registration-config`, public `POST /auth/register` and `GET /auth/verify-email?token=`. Sign-ups are always created as `viewer` with `approved`/`emailVerified` set from the config; `AuthService.login` rejects unverified/unapproved accounts *after* the password check (never leaks account state). Username/email uniqueness is case-insensitive and reported per-field. Password policy + username rules live in `@mu/shared` (`types/registration.ts` — `validatePassword`, `PASSWORD_RULE_TEXT`) so client and server can't drift; note it uses `REGISTRATION_PASSWORD_MIN_LENGTH` (8 + upper/lower/digit), distinct from the legacy `PASSWORD_MIN_LENGTH` (6) used by the self-service change form. Verification email is **stubbed** until a provider is configured — `EmailService.sendVerificationEmail` logs the link instead of throwing. |
| `users` | User management. Admin-only CRUD with last-admin protection (cannot demote/delete the only admin). `POST /users/:id/approve` clears a self-registered account's approval **and** email gates (so approval works even with email unconfigured); admin-created users bypass both gates. The list exposes `approved`/`emailVerified` so the panel can badge "pending approval" / "unverified". |
...
| `notifications` | Generic notifications — per-user (`user_id` set) and system-wide (`user_id` null, shown to all, stored once). `notifications` table (`type`, open `data` JSON, `read`). `NotificationType` enum in `@mu/shared` with a client-side per-type formatter (`utils/notification-format.ts`) → message + link. WS-pushed per-recipient (gateway routes `NOTIFICATION` by `data.userId`; clients `register` their userId). Comment replies + reactions auto-create notifications for the comment author. Admin `POST /notifications/system` creates announcements. Client: bell + dropdown in the header, toast on new ones. |
| `soundtrack` | Movie soundtrack listings (the released album tracklist) via **MusicBrainz** — keyless, no provider-platform credentials. `MusicBrainzClient` self-throttles to MB's 1 req/sec policy (serialised promise chain) with a required descriptive `User-Agent`; searches releases tagged `secondarytype:soundtrack` by title, picks the best by Official-status + release-year proximity, fetches the chosen release's recordings. `SoundtrackService` caches per movie in `soundtrack_cache` (found 30d / miss 3d TTL) and de-dupes in-flight lookups. `GET /soundtrack/:movieId` (`view:library`) → `SoundtrackDto` (`@mu/shared`). Client: collapsible **Soundtrack** section on MovieDetail, lazy-fetched on first expand. This is the album tracklist, NOT songs-in-each-scene (no free API for that). |
...
- Git remote uses SSH URL: `git@github.com:rw3iss/mu.git` (repo was renamed to `mu` but SSH URL still works)
