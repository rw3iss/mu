...
describe('findBestMatch', () => {
	...
	it('auto-applies an exact title + year match', () => {
		const res = findBestMatch({ title: 'The Matrix', year: 1999, durationMinutes: 136 }, [
		...
		expect(res.best?.confidence).toBeGreaterThanOrEqual(DEFAULT_MATCHER_CONFIG.autoApplyMin);
	});

	it('prefers the candidate whose year actually matches', () => {
		const res = findBestMatch({ title: 'Dune', year: 2021 }, [
	...
	it('treats roman numerals as their arabic equivalents', () => {
		const res = findBestMatch({ title: 'The Godfather Part II', year: 1974 }, [
	...
	it('ignores year when neither side has one (drops year weight)', () => {
		const res = findBestMatch({ title: 'Inception' }, [
	...
	it('penalises year-off-by-2 less than year-off-by-1', () => {
		const off1 = findBestMatch({ title: 'Movie', year: 2020 }, [
			cand({ title: 'Movie', year: 2019, externalId: 'a' }),
		]).best!.confidence;
		const off2 = findBestMatch({ title: 'Movie', year: 2020 }, [
			cand({ title: 'Movie', year: 2018, externalId: 'b' }),
		]).best!.confidence;
		const off3 = findBestMatch({ title: 'Movie', year: 2020 }, [
	...
	it('marks the result ambiguous when nothing clears autoApplyMin but something clears noMatchMax', () => {
		const res = findBestMatch({ title: 'Heat', year: 1995 }, [
		...
		expect(res.best!.confidence).toBeGreaterThanOrEqual(DEFAULT_MATCHER_CONFIG.noMatchMax);
	...
	it('uses popularity only as a tiebreaker, never to flip strong ranks', () => {
		...
		const res = findBestMatch({ title: 'Movie', year: 2020 }, [
		...
		const flip = findBestMatch({ title: 'The Matrix', year: 1999 }, [
	...
	it('rewards duration matches when both sides have a runtime', () => {
		const withDur = findBestMatch({ title: 'Movie', year: 2020, durationMinutes: 120 }, [
			cand({ title: 'Movie', year: 2020, runtimeMinutes: 122, externalId: 'a' }),
		]).best!;
		const withoutDur = findBestMatch({ title: 'Movie', year: 2020 }, [
