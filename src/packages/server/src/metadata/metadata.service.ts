...
export class MetadataService {
  ...
  async fetchForMovie(movieId: string) {
    ...
    const omdbData: OmdbSearchResult | null = omdbResult.status === 'fulfilled' ? omdbResult.value : null;
    ...
    const sources = [details ? 'TMDB' : null, omdbData ? 'OMDB' : null].filter(Boolean).join('+');
    this.logger.log(
