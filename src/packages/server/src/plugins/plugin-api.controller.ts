...
export class PluginApiController {
  ...
  /** Schema endpoint for client codegen */
  @Get(':name/schema')
  getSchema(@Param('name') name: string) {
    return this.apiRegistry.getSchema(name);
  }

  /** UI slot items endpoint */
  @Get(':name/ui/:slot')
  getSlotItems(@Param('name') name: string, @Param('slot') slot: string) {
    return this.uiRegistry.getSlotItems(slot);
  }

  /** Catch-all for plugin API requests */
  @All(':name/api/*')
  async handlePluginApi(
    ...
    try {
      return await this.apiRegistry.dispatch(
        name,
        method,
        subPath,
        query,
      ...
      throw new NotFoundException(
        err instanceof Error ? err.message : `No endpoint found`,
      );
    }
  }
}
