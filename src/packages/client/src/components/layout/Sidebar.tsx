...
function Icon({ d, size = 20, stroke = 1.5 }: { d: string; size?: number; stroke?: number }): JSX.Element {
  return (
...
function Icon2({ d1, d2, size = 20, stroke = 1.5 }: { d1: string; d2: string; size?: number; stroke?: number }): JSX.Element {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width={stroke} stroke-linecap="round" stroke-linejoin="round">
      <path d={d1} />
      <path d={d2} />
...
const navItems: NavItem[] = [
  ...
  },
  {
    label: 'Library',
    path: '/library',
    icon: <Icon2 d1="M4 19.5A2.5 2.5 0 0 1 6.5 17H20" d2="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z" />,
  },
  {
    label: 'Discover',
    path: '/discover',
    icon: <Icon d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20zm0 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3z" />,
  },
  {
    label: 'Playlists',
    path: '/playlists',
    ...
    <nav class={`${styles.sidebar} ${collapsed ? styles.collapsed : ''}`}>
      <div class={styles.header}>
        <button class={styles.logo} onClick={() => handleNav('/')}>
          <img src="/Mu_logo.png" alt="Mu" class={styles.logoImage} />
        </button>
        <button
          class={styles.toggle}
          onClick={onToggle}
