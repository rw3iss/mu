...
export function Plugins(_props: PluginsProps) {
	...
	const handleToggle = useCallback(async (plugin: PluginInfo) => {
		...
		try {
			...
			// Reload client-side plugins to pick up the change
			pluginClientManager.reload();
		} catch {
	...
	const handleInstall = useCallback(async (plugin: PluginInfo) => {
		...
		try {
			...
			setPlugins((prev) =>
				prev.map((p) =>
					p.name === plugin.name ? { ...p, status: 'installed' as PluginStatus } : p,
				),
			);
	...
	return (
		<div class={styles.plugins}>
			<div class={styles.header}>
				...
				<span class={styles.subtitle}>Manage extensions for your Mu instance</span>
			</div>
						...
						{plugins.map((plugin) => (
							<div key={plugin.name} class={styles.card}>
								<div class={styles.cardHeader}>
									...
									<span class={`${styles.statusBadge} ${getStatusClass(plugin)}`}>
