...
for (const f of filesNeedingDuration) {
	if (!f.file_path || !existsSync(f.file_path)) {
		probeFailed++;
		continue;
	}
	try {
		...
		if (dur > 0) {
			...
			probed++;
			if (probed % 20 === 0)
				console.log(`  Probed ${probed}/${filesNeedingDuration.length}...`);
		}
	} catch {
		probeFailed++;
	}
...
for (const dn of dirs) {
	...
	for (const q of readdirSync(dp)) {
		...
		try {
			if (!statSync(qp).isDirectory()) continue;
		} catch {
			continue;
		}
		...
		if (uniqueMatches.length === 1) {
			...
			let sizeBytes = 0;
			const segs = readdirSync(qp).filter(
				(f) => f.startsWith('segment_') && f.endsWith('.ts'),
			);
			for (const seg of segs) {
				try {
					sizeBytes += statSync(path.join(qp, seg)).size;
				} catch {}
			}
			...
			try {
				...
				console.log(
					`  ✓ ${path.basename(matchFile.file_path || '')} → ${q} (${segCount} segs, ${sizeMB} MB)`,
				);
			} catch (err: any) {
...
console.log(
	`  Coverage:          ${((totalCache.c / Math.max(totalFiles.c, 1)) * 100).toFixed(1)}%`,
);
