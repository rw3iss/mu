...
select_release() {
    step "Phase 2: Select Mu release"
    ...
    # Display releases
    echo ""
    echo -e "  ${BOLD}Available Mu Releases:${NC}"
    echo ""
...
configure_install() {
    ...
    local default_dir
    case "$PLATFORM" in
        linux)   default_dir="$HOME/mu" ;;
        macos)   default_dir="$HOME/mu" ;;
        windows) default_dir="/c/mu" ;;
    esac
    ...
    # IMDB datasets — free daily bulk download for offline rating lookups.
    echo ""
    info "IMDB datasets: download IMDB's free ratings table (~25 MB, ~1.4M titles)"
    info "and schedule a nightly sync? This lets rating lookups skip the OMDB"
    info "API quota and keeps ratings daily-fresh. Future: also pull title /"
    info "principals tables for fully-local similarity search."
    echo -en "  ${CYAN}Enable IMDB datasets nightly sync? (Y/n):${NC} "
    read -r ENABLE_IMDB_DATASETS
    ENABLE_IMDB_DATASETS="${ENABLE_IMDB_DATASETS:-y}"
    ENABLE_IMDB_DATASETS="${ENABLE_IMDB_DATASETS,,}"
    ...
    echo -e "    IMDB sync:     $([ "${ENABLE_IMDB_DATASETS}" = "y" ] && echo "enabled (~25 MB nightly)" || echo "disabled")"
    echo ""
...
download_release() {
    step "Phase 5: Downloading Mu ${SELECTED_TAG}"
    ...
    TMPDIR_CREATED=$(mktemp -d 2>/dev/null || mktemp -d -t 'mu-install')
...
generate_config() {
    ...
    local imdb_block=""
    if [ "${ENABLE_IMDB_DATASETS:-n}" = "y" ]; then
        # The runtime reads this via SettingsService.get('imdb.datasets.enabled').
        # We seed it directly in config.yml — the dotted key flattens
        # into the settings store on first boot.
        imdb_block=$'\nimdb:\n  datasets:\n    enabled: true'
    fi
    ...
    case "$PLATFORM" in
        linux)
            if command -v ufw &>/dev/null; then
                sudo ufw allow "${SERVER_PORT}/tcp" comment "Mu" && log "ufw rule added"
        ...
        macos)
            ...
            ;;
        windows)
            netsh advfirewall firewall add rule name="Mu" dir=in action=allow protocol=TCP localport="$SERVER_PORT" 2>/dev/null \
...
install_systemd_service() {
    ...
    local service_file="/etc/systemd/system/mu.service"
    local node_path
    node_path=$(which node)
    local current_user
    current_user=$(whoami)
...
show_success() {
    ...
    if [ "${INSTALL_SERVICE:-n}" = "y" ]; then
        echo -e "  ${BOLD}Service:${NC}    sudo systemctl status mu"
        echo -e "  ${BOLD}Logs:${NC}       sudo journalctl -u mu -f"
    else
