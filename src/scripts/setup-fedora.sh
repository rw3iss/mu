#!/usr/bin/env bash
#
# setup-fedora.sh — full Fedora/RHEL workstation setup for Mu.
#
...
ensure_config() {
	...
	cat > "$cfg" <<YAML
# Mu configuration — generated by setup-fedora.sh on $(date -u '+%Y-%m-%d %H:%M:%S UTC')
...
main() {
	echo -e "${BOLD}${MAGENTA}Mu — Fedora setup${NC}"
	preflight
	detect_gpu
