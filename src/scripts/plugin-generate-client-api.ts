...
async function main() {
  ...
  const methods = schema.endpoints.map((ep) => {
    ...
    const urlPath = `\`/plugins/${pluginId}/api${pathExpr}\``;

    let methodCall: string;
    if (httpMethod === 'get' || httpMethod === 'delete') {
      if (queryNames.length > 0) {
        methodCall = `return api.${httpMethod}(${urlPath}, query as Record<string, string>);`;
      } else {
        methodCall = `return api.${httpMethod}(${urlPath});`;
      }
    } else {
      methodCall = `return api.${httpMethod}(${urlPath}, body);`;
    }
