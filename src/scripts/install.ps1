...
<#
.SYNOPSIS
    Mu Install Wizard for Windows (PowerShell)
.DESCRIPTION
    Interactive installer for Mu self-hosted movie streaming server.
    Checks prerequisites, downloads a release from GitHub, builds, and configures.
#>

$ErrorActionPreference = "Stop"

$GitHubRepo = "rw3iss/mu"
...
function Show-Banner {
    ...
    Write-Host "  |                                             |" -ForegroundColor White
    Write-Host "  |          Mu Install Wizard            |" -ForegroundColor White
...
function Select-Release {
    Write-Step "Phase 2: Select Mu release"
    ...
    Write-Host ""
    Write-Host "  Available Mu Releases:" -ForegroundColor White
    Write-Host ""
...
function Configure-Install {
    ...
    $script:InstallDir = Prompt-Value "Install directory" "C:\mu"
    ...
    Write-Host ""
    Write-Info "IMDB datasets: download IMDB's free ratings table (~25 MB, ~1.4M titles)"
    Write-Info "and schedule a nightly sync? Rating lookups skip the OMDB API quota"
    Write-Info "and stay daily-fresh. Future: title / principals for fully-local search."
    $script:EnableImdbDatasets = Prompt-YesNo "Enable IMDB datasets nightly sync?" $true
    ...
    Write-Host "    IMDB sync:     $(if ($script:EnableImdbDatasets) { 'enabled (~25 MB nightly)' } else { 'disabled' })"
    Write-Host ""
...
function Download-Release {
    Write-Step "Phase 5: Downloading Mu $($script:SelectedTag)"

    $tempDir = Join-Path $env:TEMP "mu-install-$(Get-Random)"
    New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
    $zipFile = Join-Path $tempDir "mu.zip"
...
function Build-Project {
    Write-Step "Phase 6: Building Mu"
...
function Generate-Config {
    ...
    $imdbBlock = ""
    if ($script:EnableImdbDatasets) {
        # Seeded so the server's SettingsService picks it up on boot
        # and schedules the daily ratings sync.
        $imdbBlock = "`nimdb:`n  datasets:`n    enabled: true"
    }
...
function Configure-Firewall {
    ...
    try {
        New-NetFirewallRule -DisplayName "Mu" -Direction Inbound `
...
function Show-Success {
    ...
    Write-Host "  |   Mu installed successfully!          |" -ForegroundColor Green
    Write-Host "  +=============================================+" -ForegroundColor Green
    Write-Host ""
