...
export default class RottenTomatoesPlugin implements IPlugin {
	...
	async getScore(title: string, year?: number): Promise<RottenTomatoesScore> {
		...
		if (score.tomatoScore !== null) {
			this.context.logger.log(`Rotten Tomatoes score for "${title}": ${score.tomatoScore}%`);
		} else {
			this.context.logger.debug(`No Rotten Tomatoes score found for "${title}"`);
		}
	 ...
	 */
	private async lookupViaOmdb(title: string, year?: number): Promise<RottenTomatoesScore> {
	 ...
	 */
	private buildEmptyScore(title: string, year?: number): RottenTomatoesScore {
		return {
			title,
	...
	private getApiKey(): string {
		...
		const apiKey =
			(this.context.config.apiKey as string | undefined) ??
