...
export default class TmdbMetadataPlugin implements IPlugin {
  ...
  async searchMovie(
    ...
    const includeAdult =
      (this.context.config.includeAdult as boolean) ?? false;
  ...
  private async handleMovieNeedsMetadata(
    ...
    this.context.logger.log(
      `Metadata updated for "${title}" (TMDB ID: ${details.id})`,
    );
  ...
  private getApiKey(): string {
    ...
    if (!apiKey) {
      throw new Error(
        'TMDB API key is not configured. Set it in the plugin settings.',
      );
    }
